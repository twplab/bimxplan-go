...
class BEPErrorHandler {
   ...
   */
  log(level: BEPLogEntry['level'], action: string, message: string, context: Record<string, any> = {}) {
   ...
   */
  handleError(error: Error | string, action: string, context: Record<string, any> = {}): BEPError {
 ...
 */
export function withBEPErrorHandling<T extends any[], R>(
